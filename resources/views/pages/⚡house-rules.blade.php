...
new #[Title('House Rules')] class extends Component {
}; ?>

<div class="flex h-full w-full flex-1 flex-col gap-6 pb-8">

    {{-- Hero banner --}}
    <div class="relative overflow-hidden rounded-2xl bg-linear-to-br from-amber-500 via-orange-500 to-rose-500 p-8 text-white shadow-lg">
        ...
        <div class="relative">
            <div class="mb-1 flex items-center gap-2 opacity-80">
                ...
                <span class="text-xs font-semibold uppercase tracking-widest">{{ __('house-rules.title') }}</span>
            </div>
            <h1 class="text-3xl font-bold tracking-tight">{{ __('house-rules.title') }}</h1>
            <p class="mt-1 text-orange-100">{{ __('house-rules.subtitle') }}</p>
        </div>
    </div>

    {{-- Sections --}}
    <div class="flex flex-col gap-4">
        @foreach(__('house-rules.sections') as $section)
                        ...
                        <ul class="mt-2 space-y-1.5">
                            @foreach($section['rules'] as $rule)
                                <li class="flex items-start gap-2">
                                    <flux:icon name="check" class="mt-0.5 size-4 shrink-0 text-amber-500 dark:text-amber-400" />
                                    <flux:text class="text-neutral-600 dark:text-neutral-400">{{ $rule }}</flux:text>
                                </li>
                            @endforeach
                        </ul>
                    </div>
